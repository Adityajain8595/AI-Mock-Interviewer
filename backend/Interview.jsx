...
import { Upload, Mic, Briefcase, BarChart, Loader2, CheckCircle, XCircle, User, Mail, Phone, Link, FileText, GraduationCap, Code, HeartHandshake, Award, Plus, Trash2, ChevronRight, ChevronLeft, SendHorizonal } from "lucide-react";
...
export default function App() {
  ...
  const sendToBackend = async (text) => {
    ...
    try {
      ...
      speak(data.reply); // Play the AI's response as audio
  ...
  // ================= Renders =================
  return (
    <div className="min-h-screen font-inter text-white overflow-hidden relative">
      ...
      <div className="min-h-screen relative z-10 flex items-center justify-center p-6">
        ...
        >
          ...
          {step === 1 && (
            <div className={sectionContainer}>
              {loading ? (
                ...
                <motion.div initial={{ x: 200, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="space-y-6">
                  <h2 className={headingStyle}>Experience & Projects</h2>
                ...
                <motion.div initial={{ x: 200, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="space-y-6">
                  <h2 className={headingStyle}>Skills & Certifications</h2>
                  ...
                  <div className="flex justify-between items-center mt-8">
                    ...
                    <button onClick={() => setStep(2)} className="bg-gradient-to-r from-purple-600 to-pink-600 px-8 py-3 rounded-xl font-bold flex items-center transition-transform duration-300 transform hover:scale-105">
          ...
          {/* Step 3: Interview */}
          {step === 3 && (
            <motion.div initial={{ y: 200, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className={`${sectionContainer} flex flex-col h-[85vh]`}>
              <h2 className="text-2xl font-bold mb-4 text-center text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600">Mock Interview</h2>
              <div ref={chatContainerRef} className="flex-1 overflow-y-auto space-y-4 mb-6 pr-4 custom-scrollbar">
                {chat.map((msg, idx) => (
                  <motion.div
                    key={idx}
              ...
              <div className="flex items-center space-x-4">
                <form onSubmit={handleTextSubmit} className="flex-1 flex items-center space-x-4">
                  <input
                    ...
                    className="flex-1 bg-gray-800 rounded-full py-3 px-6 text-white outline-none border border-gray-700 focus:border-purple-500"
                  />
                  <motion.button 
                    whileTap={{ scale: 0.9 }}
                    type="submit"
              ...
              <div className="flex justify-center mt-6">
                {interviewCompleted && (
                  <button
                    ...
                    className="bg-gray-700 hover:bg-gray-600 px-8 py-3 rounded-full font-bold flex items-center transition-all duration-300 transform hover:scale-105 shadow-lg"
                  >
          ...
          {/* Step 4: Evaluation */}
          {step === 4 && evaluation && (
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className={sectionContainer}>
              <h2 className={headingStyle}>
                <BarChart size={40} className="inline mr-3" /> Evaluation Report
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                {Object.entries(evaluation.scores).map(([metric, score], i) => (
                  <div key={i} className="flex flex-col items-center p-4 bg-gray-800 rounded-xl shadow-lg transition-transform duration-300 hover:scale-105">
                    <CircularProgressbarSVG value={score} text={`${score}%`} color="pink" />
